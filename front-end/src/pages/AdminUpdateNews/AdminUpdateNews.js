...
import './adminUpdateNews.css';

export default function AdminUpdateNews() {
    ...
    useEffect(() => {
        if(newsData.status === "success") {
            setValue('properties', newsData.data.news.properties);
            setValue('title', newsData.data.news.title);
            setValue('sapo', newsData.data.news.sapo);
            setSrc(newsData.data.news.iconUrl);
            setContent(newsData.data.news.content);
            setDisabled(false);
        }
    }, [newsData]);
    ...
    const onSubmit = async values => {
        ...
        if (values.iconUrl || newsData.data.news.iconUrl) {
            try {
                const res = await request({
                    url: `/api/news/${newsId}`,
                    method: 'PUT',
